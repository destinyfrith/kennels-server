...
def get_all_employees():
    ...
    with sqlite3.connect("./kennel.sqlite3") as conn:
        ...
        # We import up top to access location, then add properties we want
        # Join tables where properties match, then add instance + dictionary below
        db_cursor.execute("""
        SELECT
            e.id,
            e.name,
        ...
        for row in dataset:
            ...
            # has to match the lines in dbcursor (include _)
            location = Location(
                row['id'], row['location_name'], row['location_address'])

            # Add the dictionary rep of the customer to the animal
